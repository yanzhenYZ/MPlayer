...
void IJK_GLES2_Renderer_Degress(IJK_GLES2_Renderer *renderer, int degress)
{
    renderer->degress = degress;
}
...
#if 1
static void IJK_GLES2_Renderer_TexCoords_Left(IJK_GLES2_Renderer *renderer)
{
    renderer->texcoords[0] = 0.0f;
    renderer->texcoords[1] = 0.0f;
    renderer->texcoords[2] = 0.0f;
    renderer->texcoords[3] = 1.0f;
    renderer->texcoords[4] = 1.0f - renderer->cropRight;
    renderer->texcoords[5] = 0.0f;
    renderer->texcoords[6] = 1.0f - renderer->cropRight;
    renderer->texcoords[7] = 1.0f;
}

static void IJK_GLES2_Renderer_TexCoords_180(IJK_GLES2_Renderer *renderer)
{
    renderer->texcoords[0] = 1.0f - renderer->cropRight;
    renderer->texcoords[1] = 0.0f;
    renderer->texcoords[2] = 0.0f;
...
static void IJK_GLES2_Renderer_TexCoords_Right(IJK_GLES2_Renderer *renderer)
{
    renderer->texcoords[0] = 1.0f - renderer->cropRight;
    renderer->texcoords[1] = 1.0f;
    renderer->texcoords[2] = 1.0f - renderer->cropRight;
    renderer->texcoords[3] = 0.0f;
...
{
    if (renderer->degress == 90) {
        IJK_GLES2_Renderer_TexCoords_Right(renderer);
    } else if (renderer->degress == 180) {
        IJK_GLES2_Renderer_TexCoords_180(renderer);
    } else if (renderer->degress == 270) {
        IJK_GLES2_Renderer_TexCoords_Left(renderer);
    } else {//old - only-Normal
        //IJK_GLES2_Renderer_TexCoords_Nomarl(renderer);
        IJK_GLES2_Renderer_TexCoords_cropRight(renderer, renderer->cropRight);
    }
}
#else //ijk origion
static void IJK_GLES2_Renderer_TexCoords_reset(IJK_GLES2_Renderer *renderer)
...
#if 1
        if (renderer->degress == 90 || renderer->degress == 270) {
            if (renderer->frame_width   != visible_height    ||
                renderer->frame_height  != visible_width   ||
                renderer->frame_sar_num != overlay->sar_num ||
                renderer->frame_sar_den != overlay->sar_den) {
                
                renderer->frame_width   = visible_height;
                renderer->frame_height  = visible_width;
                renderer->frame_sar_num = overlay->sar_num;
                renderer->frame_sar_den = overlay->sar_den;
