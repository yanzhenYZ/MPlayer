...
//added by yanzhen yanzhen -- FUNC_0004 2020-05-20
static long long audio_recv_bytes_;
static long long video_recv_bytes_;
static long long audio_stat_time_;
static long long video_stat_time_;
static long long last_audio_bitrate_;
static long long last_video_bitrate_;
//end
...
{
    //added by yanzhen yanzhen -- FUNC_0003 2020-05-20
    audio_recv_bytes_ = 0;
    video_recv_bytes_ = 0;
    audio_stat_time_= SDL_GetTickHR();
    video_stat_time_= SDL_GetTickHR();
    last_audio_bitrate_ = 0;
    last_video_bitrate_ = 0;
    //end
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
...
//      of the seek_pos/seek_rel variables
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            //added by yanzhen yanzhen -- FUNC_0004 2020-05-20
            audio_recv_bytes_ += pkt->size;
            //end
            packet_queue_put(&is->audioq, pkt);
        } else if (pkt->stream_index == is->video_stream && pkt_in_play_range
                   && !(is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))) {
            //added by yanzhen yanzhen -- FUNC_0004 2020-05-20
            video_recv_bytes_ += pkt->size;
            //end
...
long long getAudioBitrate(){
    long long now = SDL_GetTickHR();
    if(now-999>audio_stat_time_){
        last_audio_bitrate_ = audio_recv_bytes_*1000*8/(now-audio_stat_time_);
        audio_recv_bytes_ = 0;
        audio_stat_time_ = now;
    }
    return last_audio_bitrate_;
}

long long getVideoBitrate() {
    long long now = SDL_GetTickHR();
    if(now-999>video_stat_time_){
        last_video_bitrate_ = video_recv_bytes_*1000*8/(now-video_stat_time_);
        video_recv_bytes_ = 0;
        video_stat_time_ = now;
    }
    return last_video_bitrate_;
}
//end
...
//added by yanzhen yanzhen -- FUNC_0004 2020-05-20
        case FFP_PROP_INT64_AUDIO_BIT_RATE:
            return getAudioBitrate();
        case FFP_PROP_INT64_VIDEO_BIT_RATE:
