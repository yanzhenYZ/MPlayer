...
//added by yanzhen yanzhen --- FUNC_0007
    BOOL _inBackground;
    BOOL _interruptEnd;
}
...
- (void)audioSessionInterrupt:(NSNotification *)notification
{
    ...
    switch (reason) {
        case AVAudioSessionInterruptionTypeBegan: {
            NSLog(@"yanzhenPlayerController:audioSessionInterrupt: begin\n");
            switch (self.playbackState) {
                case IJKMPMoviePlaybackStatePaused:
                case IJKMPMoviePlaybackStateStopped:
        ...
        case AVAudioSessionInterruptionTypeEnded: {
            NSLog(@"yanzhenPlayerController:audioSessionInterrupt: end : %d", _inBackground);
            if (_activeResume) {
                if (!_inBackground) {
...
{
    NSLog(@"yanzhenPlayerController:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
}

- (void)applicationDidBecomeActive
{
    NSLog(@"yanzhenPlayerController:applicationDidBecomeActive: %d:%d", (int)[UIApplication sharedApplication].applicationState, _interruptEnd);
    _inBackground = NO;
    if (_interruptEnd && _activeResume) {
        _interruptEnd = NO;
        [[IJKAudioKit sharedInstance] setActive:YES];
        if (_playingBeforeInterruption) {
            [self play];
...
{
    NSLog(@"yanzhenPlayerController:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
    _inBackground = YES;
//    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    NSLog(@"yanzhenPlayerController:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_pauseInBackground) {
            [self pause];
        }
...
{
    NSLog(@"yanzhenPlayerController:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_pauseInBackground) {
            [self pause];
        }
