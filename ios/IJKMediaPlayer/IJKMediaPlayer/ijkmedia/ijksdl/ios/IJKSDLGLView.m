...
@property(nonatomic) int degress;
@end
...
- (void)rotateDegress:(int)degress {
    _degress = degress;
    if (_renderer) {
        IJK_GLES2_Renderer_Degress(_renderer, degress);
    }
}
...
- (BOOL)setupRenderer: (SDL_VoutOverlay *) overlay
{
    ...
    if (!IJK_GLES2_Renderer_isValid(_renderer) ||
        ...
        IJK_GLES2_Renderer_Degress(_renderer, _degress);
        if (!IJK_GLES2_Renderer_isValid(_renderer))
            return NO;
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay
{
    ...
    //yanzhen 2020-06-30 MSB 缩放功能 M001
    if (self.superview.superview) {
        [[self eaglLayer] setContentsScale:_scaleFactor * self.superview.superview.transform.a];
    } else {
